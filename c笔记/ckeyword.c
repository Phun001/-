...
条件表达式：x = (y < 0)? -y: y;
max = (a > b)? a: b; // 等价于a大于b为真，max = a, a大于b为假，max = b; 
...
"第三次更新"
/* 指针的一点心得 */
内存是连续字组成的，不同类型数据，需要占用不同宽度的字，每当你声明一种数据类型，就会在内存中开辟一定宽度的内存和与之对应的地址！
int n; // 就会创建一个4个字节的宽度，并且有相对应的地址
n = 4; // 会把4存在n对应的地址的宽度内

int m; // 一样也会创建4个字节的宽度内存，并且有对应的地址
m = n; // m对应内存空间也会存储4
m = m + 1; // 操作的是m对应内存地址中的值加一，不会对n造成影响，也就是m的值并不引用n的而是赋值n的值，存储在m对应内存地址的内存中

指针又是什么？
指针是一种特殊的数据类型，也需要声明， 指针操作的地址
如： int *ph;  // ph就是指针，但是ph并不存储‘值’， 存储的是地址
ph = &n;  // 此时对(ph + 1)就是对&n的地址偏移一个int存储单元，至于会是神马值，很难说，只能看他加上对应值地址存储值
*ph + 1; // 由于ph存储的是n的地址所以对*ph + 1就等价于 n + 1, *ph += 1;等价于 n += 1;
int temp; // 创建一个int类型
temp = *ph;  // *ph是对地址的解引，会获取对应地址的值，这时候就等价于 temp = n;
